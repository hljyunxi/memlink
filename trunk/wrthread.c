...
wrthread_create()
{
    ...
    //DINFO("WRThread create start.\n");
    wt = (WRThread*)zz_malloc(sizeof(WRThread));
    if (NULL == wt) {
        DERROR("wrthread malloc error.\n");
        MEMLINK_EXIT;
    }
    memset(wt, 0, sizeof(WRThread));
    ...
    g_runtime->wrthread = wt;

    DINFO("create WRThread ok!\n");

    return wt;
}

void
wrthread_loop(WRThread *wt)
{
    event_base_loop(wt->base, 0);
}

void
wrthread_destroy(WRThread *wt)
{
    zz_free(wt);
}
