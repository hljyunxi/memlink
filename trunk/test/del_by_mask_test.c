...
int main()
{
	...
	if (ret != MEMLINK_OK)
	{
		DINFO("range error, key:%s, mask:%s\n", buf, "");
		return -3;
	}
	if(rs.count != 0)
	{
		DINFO("error! rs.count must be 0! rs.count:%d\n", rs.count);
		return -1;
	}
	...
	if(ret != MEMLINK_OK)
	{
		DINFO("stat err key:%s, ret:%d\n", buf, ret);
		return -1;
	}

	if(st.data_used != 0)
	{
		DINFO("error data_used must be 0, st.data_used:%d\n", st.data_used);
		//return -1;
	}
	...
	for(i = 0; i < 4; i++)
	{
		...
		if(rs.count != 0)
		{
			DINFO("err! rs.count must be 0! rs.count:%d\n", rs.count);
			return -1;
		}
